...
const Maincontent = () => {
  ...
  const getPaginationButtons = () => {
    ...
    if (currPage - 2 < 1) {
      endPage = Math.min(totalPages, endPage + (2 - currPage - 1));
    }
    if (currPage + 2 > totalPages) {
      startPage = Math.min(1, startPage - (2 - totalPages - currPage));
    }
      ...
      <div className="mb-5">
        <div className="flex flex-col sm:flex-row justify-between items-center">
          <div className="relative mb-5 mt-5">
            <button onClick={()=>setDropDownOpen(!dropdownOpen) }className="border px-4 py-2 rounded-full items-center">
            ...
            {getPaginationButtons().map((page) => (
              <button
                ...
                className={`border px-4 py-2 mx-1 rounded-full ${
                  page === currPage ? "text-white bg-black" : ""
                }`}
              >{page}</button>
            ))}
          </div>
